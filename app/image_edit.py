...
_EDIT_INSTRUCTION = (
    ...
    "只允许做三类调整：1）二次构图裁剪；2）修复画面中不美观的瑕疵和杂物；"
    "3）曝光、对比度、白平衡、高光阴影、色彩、锐化等参数级调整。"
    "严禁替换、重绘或生成画面内容，不要添加或删除主体元素，不要改变拍摄场景，"
    "画面主体、内容和构图必须与原图保持一致；"
    "调整幅度以建议为准，做适度、自然的影调和色彩优化，"
    "不要过度强化风格，也不要使用极端参数导致画面失真：\n\n"
)


def extract_suggestions(markdown: str) -> str:
    """从点评 markdown 中提取「后期调整建议」小节；提取不到返回全文。
    标题允许带编号或括号补充，如「## 五、后期调整建议（……）」。"""
    match = re.search(r"^#{1,6}\s*[^\n#]*后期调整建议[^\n]*\n(.*?)(?=^#{1,6}\s|\Z)", markdown, re.M | re.S)
